...
static inline void* bench_memcpy(void* dst, const void* src, bench_size_t n){
	if (!(dst && src))
	{
		puts("Assertion failed:!(dst && src)");
		return ;
	}
  const char* s;
  char* d;
  if((const char*)src+n>dst&&src<dst){
    s=(const char*)src+n;
    d=(char*)dst+n;
    while(n-->0)*--d=*--s;
  }
