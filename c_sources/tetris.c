...
void print_game_area() {
    ...
    displayChar(1 + Width + Py_x + 5, 14, 'S', WHITE);
    displayChar(2 + Width + Py_x + 5, 14, 'c', WHITE);
    displayChar(3 + Width + Py_x + 5, 14, 'o', WHITE);
    displayChar(4 + Width + Py_x + 5, 14, 'r', WHITE);
    displayChar(5 + Width + Py_x + 5, 14, 'e', WHITE);
    displayChar(6 + Width + Py_x + 5, 14, ':', WHITE);
    displayChar(7 + Width + Py_x + 5, 14, sco_s, WHITE);
    displayChar(8 + Width + Py_x + 5, 14, sco_g, WHITE);

    char levl_s = level / 10 + '0';
    if(levl_s == '0')levl_s=' ';
    char levl_g = level % 10 + '0';
    displayChar(1 + Width + Py_x + 5, 16, 'L', WHITE);
    displayChar(2 + Width + Py_x + 5, 16, 'e', WHITE);
    displayChar(3 + Width + Py_x + 5, 16, 'v', WHITE);
    displayChar(4 + Width + Py_x + 5, 16, 'e', WHITE);
    displayChar(5 + Width + Py_x + 5, 16, 'l', WHITE);
    displayChar(6 + Width + Py_x + 5, 16, ':', WHITE);
    displayChar(7 + Width + Py_x + 5, 16, levl_s, WHITE);
    displayChar(8 + Width + Py_x + 5, 16, levl_g, WHITE);
    //printf("Score: %d\n", score);
    //printf("Level: %d\n", level);
}
...
void tetris_exec() {
    ...
    while (1) {
        ...
        if (can_move(0, 1, block_rotation)) {
            ...
            clear_lines();
            spawn();
            if (!can_move(0, 0, block_rotation)) {
                break;
            }
            // 检查游戏是否结束（即游戏区域顶部有方块）
            int game_over = 0;
