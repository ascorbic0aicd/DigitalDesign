...
static int same(const unsigned char *src, bench_size_t n)
{
	while(n > 0 && *(src + n) == *src)
		n--;
	return n == 0 ? 1 : 0;
...
static __inline void fast_write(ui32 f, void *dst, bench_size_t bytes)
{
  for (bench_size_t i = 0; i != bytes; i ++) {
    ((char*)dst)[i] = ((char*)&f)[i];
  }
}


bench_size_t qlz_size_decompressed(const char *source)
{
	ui32 n, r;
	n = (((*source) & 2) == 2) ? 4 : 1;
...
bench_size_t qlz_size_compressed(const char *source)
{
	ui32 n, r;
	n = (((*source) & 2) == 2) ? 4 : 1;
...
bench_size_t qlz_size_header(const char *source)
{
	bench_size_t n = 2*((((*source) & 2) == 2) ? 4 : 1) + 1;
	return n;
}
...
static bench_size_t qlz_compress_core(const unsigned char *source, unsigned char *destination, bench_size_t size, qlz_state_compress *state)
{
	...
	while(src <= last_matchstart)
	{
			...
			if (cached == 0 && o != OFFSET_BASE && (src - o > MINOFFSET || (src == o + 1 && lits >= 3 && src > source + 3 && same(src - 3, 6))))
			{
				...
				else
				{
					...
					if(*(o + (src - old_src)) == *src)
					{
						...
						if(*(o + (src - old_src)) == *src)
						{
							bench_size_t q = last_byte - UNCOMPRESSED_END - (src - 5) + 1;
							bench_size_t remaining = q > 255 ? 255 : q;
							src++;
							while(*(o + (src - old_src)) == *src && (bench_size_t)(src - old_src) < remaining)
								src++;
						}
					}
...
#elif QLZ_COMPRESSION_LEVEL >= 2
		{
			...
			unsigned char c;
			bench_size_t remaining = (last_byte - UNCOMPRESSED_END - src + 1) > 255 ? 255 : (last_byte - UNCOMPRESSED_END - src + 1);
			(void)best_k;
...
static bench_size_t qlz_decompress_core(const unsigned char *source, unsigned char *destination, bench_size_t size, qlz_state_decompress *state, const unsigned char *history)
{
	...
	for(;;)
	{
		...
		if ((cword_val & 1) == 1)
		{
			...
			cword_val = cword_val >> 1;
			hash = (fetch >> 4) & 0xfff;
			offset2 = (const unsigned char *)(bench_size_t)state->hash[hash].offset;
...
bench_size_t qlz_compress(const void *source, char *destination, bench_size_t size, qlz_state_compress *state)
{
	bench_size_t r;
	ui32 compressed;
	bench_size_t base;
...
bench_size_t qlz_decompress(const char *source, void *destination, qlz_state_decompress *state)
{
	bench_size_t dsiz = qlz_size_decompressed(source);
